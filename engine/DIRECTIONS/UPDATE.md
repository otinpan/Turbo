...
A command is a user-defined action that runs with `UpdateContext`.
...
When you want to create new system, you create new struct using `UpdateSystem` trait. 
```rust
#[derive(Clone, Debug)]
...
```
And then, register it. All of pipelines between System and Resources like Primitive, Model, timer, input, Components... are provided by APIs below.
```rust
