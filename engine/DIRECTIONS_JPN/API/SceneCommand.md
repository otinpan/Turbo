# SceneCommand API
Sceneの変更など、Sceneに関する処理を行う場合、`SceneCommand API`を通してScene操作を行います。

## Sceneの変更
`set_current_scene()`で遷移先のSceneを指定し、移動することが出来ます。
```rust
...
`BasicFieldScene`でSpaceを押すと、`Basic3dScene`に遷移することが出来ます。

![](../../../assets/tutorial_basic_field_scene.png)
![](../../../assets/tutorial_basic_3d_scene.png)

遷移すると、前のSceneで登録されていた`UpdateSystem`や`Command`はリセットされます。また、`SceneOwned`のコンポーネントが付いたEntityはすべて削除されます。
